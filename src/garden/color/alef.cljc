...
;            (/ (- mx mn)
;               (- 510 mx mn)))]
;    #?@(:clj  (Hsl. (mod (double h) 360) (* 100.0 s) (* 100.0 l))
;        :cljs (Hsl. (mod (float h) 360) (* 100.0 s) (* 100.0 l)))))
;
;;;; From Hsl
;
;(defn ^Rgb hsl->rgb
...
;        l (/ l 100.0)
;        ;; C = (1 - |2L - 1|) * s
;        c #?@(:clj  (* (- 1 (Math/abs (double (- (* 2 l) 1)))) s)
;              :cljs (* (- 1 (Math/abs (- (* 2 l) 1))) s))
;        ;; X = C * (1 - |(H/60) mod 2 - 1|)
;        x #?@(:clj  (* c (- 1 (Math/abs (double (- 1 (mod (/ h 60.0) 2))))))
;              :cljs (* c (- 1 (Math/abs (- 1 (mod (/ h 60.0) 2))))))
;        ;; m = L - C/2
;        m (- l (/ c 2.0))
;        [r' g' b']
;        (cond
...
;        r #?@(:clj  (Math/round ^Double (* 0xff (+ r' m)))
;              :cljs (Math/round (* 0xff (+ r' m))))
;        g #?@(:clj  (Math/round ^Double (* 0xff (+ g' m)))
;              :cljs (Math/round (* 0xff (+ g' m))))
;        b #?@(:clj  (Math/round ^Double (* 0xff (+ b' m)))
;              :cljs (Math/round (* 0xff (+ b' m))))]
;    (Rgb. r g b)))
;
;;; ---------------------------------------------------------------------
;;; Arithmetic
...
;   (tetrad color 90))
;  ([color angle]
;   (let [degrees #?@(:clj  (max 1 (min 90 (Math/abs (double angle))))
;                     :cljs (max 1 (min 90 (Math/abs angle))))
...
;      #?@(:clj  (clj// (bit-and (bit-shift-right l 24) 0xff) 255.0)
;          :cljs (/ (bit-and (bit-shift-right l 24) 0xff) 255.0))
;
;
;      1.0))
;
...
;    (let [[r g b] #?@(:clj  (map #(Integer/parseInt ^String %) ns)
;                      :cljs (map #(js/parseInt ^String %) ns))]
;      (Rgb. r g b))
...
;    (let [[r g b] #?@(:clj  (map #(Integer/parseInt ^String %) [r g b])
;                      :cljs (map #(js/parseInt ^String %) [r g b]))
;
;          a #?@(:clj  (Double. ^String a)
;                :cljs (js/parseFloat a))]
;      (Rgba. r g b a))
;    (throw (ex-info (str "Invalid rgb string: " (pr-str s))
;                    {:given    s
...
;    (let [[h s] #?@(:clj  (map #(Integer/parseInt ^String %) [h s])
;                    :cljs (map #(js/parseInt ^String %) [h s]))
;          l #?@(:clj  (Double. ^String l)
;                :cljs (js/parseFloat l))]
;      (Hsl. h s l))
