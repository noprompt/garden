...
(deftest parse-vector-test
  (testing "parse-vector"
    (is (= [:css/rule
           ...
           (garden.parse.alef/parse-vector [:a])))

    (is (= [:css/rule
            [:css.selector/simple "a|b"]
            [:css.declaration/block]]
           (garden.parse.alef/parse-vector [:a/b])))
  ...
  (testing "parse-hash-map"
    (is (= [:css.declaration/block
            [:css/declaration
             ...
             [:css.declaration/value
              [:css/identifier "b"]]]]
           (garden.parse.alef/parse-hash-map {:a :b})))
    ...
    (is (= [:css.declaration/block
            [:css/declaration
             ...
             [:css.declaration/value
              [:css/identifier "b"]]]]
           (garden.parse.alef/parse-hash-map {:x/a :b})))))
